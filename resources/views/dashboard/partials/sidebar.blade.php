...
<div :class="{ 'w-64': sidebarOpen, 'w-20': !sidebarOpen, 'transition-all duration-300 ease-in-out': isLoaded }" class="w-64 bg-white dark:bg-gray-800 shadow-2xl flex flex-col h-screen sticky top-0 z-30 overflow-hidden">

    <!-- 1. Seção do Logo + Toggle (Topo) -->
    <div class="flex items-center px-3 pt-4 pb-3 flex-shrink-0 border-b border-gray-200 dark:border-gray-700"
         :class="{ 'justify-between': sidebarOpen, 'justify-center': !sidebarOpen }">
        
        <!-- Logo (visível quando expandido) - usa opacity para evitar flicker -->
        <a href="{{ route('aluno') }}" 
           class="block overflow-hidden"
           :class="{ 
               'flex-1 max-w-[180px] opacity-100': sidebarOpen, 
               'w-0 opacity-0 absolute': !sidebarOpen,
               'transition-all duration-300': isLoaded 
           }">
            <img src="{{ asset('images/unicentro-logo-new.png') }}" alt="Unicentro" class="h-14 w-auto object-contain" />
        </a>
        
        <!-- Ícone pequeno quando minimizado - usa opacity para evitar flicker -->
        <a href="{{ route('aluno') }}" 
           class="block"
           :class="{ 
               'opacity-0 w-0 absolute': sidebarOpen, 
        ...
        <ul class="space-y-2 px-4">
            <li>
                <a href="{{ route('aluno') }}" title="Início" class="flex items-center gap-3 py-2.5 px-3 rounded-lg transition-all duration-200 {{ request()->routeIs('aluno') ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }}" :class="{'justify-center': !sidebarOpen}">
                    ...
                    <span class="font-medium whitespace-nowrap" :class="{ 'opacity-100': sidebarOpen, 'opacity-0 w-0': !sidebarOpen, 'transition-opacity duration-300': isLoaded }">Início</span>
                </a>
            </li>
            <li>
                <a href="{{ route('favorites.index') }}" title="Favoritos" class="flex items-center gap-3 py-2.5 px-3 rounded-lg transition-all duration-200 {{ request()->routeIs('favorites.index') ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }}" :class="{'justify-center': !sidebarOpen}">
                    ...
                    <span class="font-medium whitespace-nowrap" :class="{ 'opacity-100': sidebarOpen, 'opacity-0 w-0': !sidebarOpen, 'transition-opacity duration-300': isLoaded }">Favoritos</span>
                </a>
            </li>
            <li>
                <button @click.prevent="showSubmissions = true" title="Meus Envios" class="w-full flex items-center gap-3 py-2.5 px-3 rounded-lg transition-all duration-200 text-left text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700" :class="{'justify-center': !sidebarOpen}">
                    ...
                    <span class="font-medium whitespace-nowrap" :class="{ 'opacity-100': sidebarOpen, 'opacity-0 w-0': !sidebarOpen, 'transition-opacity duration-300': isLoaded }">Meus envios</span>
                </button>
            </li>
        </ul>
    </nav>
    ...
    <div class="flex-shrink-0 border-t border-gray-200 dark:border-gray-700 space-y-4 p-4" 
         :class="{ 'p-4': sidebarOpen, 'p-2': !sidebarOpen }">
        ...
        <a href="{{ route('profile.edit') }}" title="Perfil" class="flex items-center gap-3" :class="{ 'justify-center': !sidebarOpen }">
            ...
            <div class="flex-1 overflow-hidden" 
                 :class="{ 'opacity-100 max-w-full': sidebarOpen, 'opacity-0 max-w-0 hidden': !sidebarOpen, 'transition-all duration-300': isLoaded }">
        ...
        <div class="flex items-center gap-2"
             :class="{ 'flex-row': sidebarOpen, 'flex-col': !sidebarOpen }">
